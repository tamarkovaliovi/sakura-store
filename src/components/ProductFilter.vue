<template>
  <div class="min-h-screen flex flex-col font-sans bg-background">
    ...
    <div
      class="relative bg-white pt-12 pb-10 px-4 sm:px-6 lg:px-8 text-center shadow-sm overflow-hidden"
    >
      <div
        class="absolute inset-0 flex items-center justify-center opacity-5 pointer-events-none select-none"
      >
        <span
          class="text-[10rem] font-bold text-blue-900 uppercase tracking-widest transform scale-110"
        >
          Sakura
        </span>
      </div>
    ...
    <main class="flex-grow container mx-auto px-4 py-8">
      <div v-if="loading" class="text-center text-gray-500 py-20">
        <div
          class="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-900 mx-auto mb-4"
        ></div>
      ...
      >
        ...
        <p class="text-gray-500 mb-6">
          Farklı bir anahtar kelime deneyebilir veya tüm ürünlere dönebilirsiniz.
        </p>
      ...
      <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        ...
        >
          <div class="relative h-64 overflow-hidden bg-gray-100">
            ...
            <button
              class="absolute bottom-4 right-4 bg-white text-dark p-3 rounded-full shadow-lg transition-all duration-300 hover:bg-blue-900 hover:text-white hover:scale-110 group-hover:bottom-6"
            >
              <svg
...
const formatImage = (imgurl) => {
  ...
  if (
    cleaned.includes("place") ||
    cleaned.includes("canvas") ||
